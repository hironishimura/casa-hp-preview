...
| `inc/content.php` 数値タイル | 「4% 建築家の設計料」を削除（4つ→3つ） |
...
**CSSも1か所直しています。** 数値タイルは `grid-template-columns:repeat(4,1fr)` の4列固定だったため、
3つに減らすと右端が空きます。`repeat(auto-fit,minmax(200px,1fr))` に変更し、
項目数に応じて列数が決まるようにしました（3つで横幅いっぱいに並ぶことを実測確認済み）。
