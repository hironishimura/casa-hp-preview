...
最終更新：2026-08-17　テーマ v1.4.2

## ひとことで言うと

WordPressテーマは**完成済み**。最新は **v1.4.2**。
本番 d-casa.jp は **v1.4.1 が稼働中**（2026-08-17 に実測）。**施工例の絞り込みタブが404のまま**で、v1.4.2 への入れ替えが必要です。
...
| 項目 | 状態 |
|---|---|
| WordPressテーマ `designcasa-shome` | **完成**（v1.4.2） |
...
| **d-casa.jp への設置** | **設置済み**（**v1.4.1 が稼働中**。v1.4.2 への入れ替え待ち） |
| 施工例の絞り込みタブ（`/works/feature/...`） | **本番で404のまま**。v1.4.2 で修正。下記参照 |
...
**⓪【最優先】本番を v1.4.2 に入れ替える（施工例の絞り込みタブが404）**

**パーマリンク設定の保存では直りません。** 原因はリライト規則の消失ではなく、
規則どうしの優先順位だったためです（詳細は「v1.4.2 で直したこと」）。
**テーマZIPの入れ替えが必須**です。入れ替え後に次が `200` になれば完了：
...
URL解決の順序と判定ロジックは、テーマの実コードを読み込んでPHPで検証済み
（`works/feature/hiraya/`・ページ送り・`spec/category/…`・既存のCPT URL が壊れないこと、
判定ロジック4パターン）。
※ **WordPress 実機では未検証**（ローカルにWP環境がないため）。本番反映後に
`/works/feature/hiraya/` が **`-L` を付けずに** 200 になることを必ず確認すること。
