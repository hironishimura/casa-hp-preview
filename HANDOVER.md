...
最終更新：2026-08-08　テーマ v1.4.1

## ひとことで言うと

WordPressテーマは**完成済み**。GitHub Pages のプレビューは**最新の v1.4.1**。
本番 d-casa.jp は **v1.4.0 が稼働中**（2026-08-08 に実測して確認）。
...
| 項目 | 状態 |
|---|---|
| WordPressテーマ `designcasa-shome` | **完成**（v1.4.1） |
...
| **d-casa.jp への設置** | **設置済み**（**v1.4.0 が稼働中**。v1.4.1 への入れ替え待ち） |
...
**⓪【最優先・所要10秒】本番の施工例タブが404のままなら、お客さまに直していただく**

d-casa.jp の **設定 → パーマリンク設定 → 「変更を保存」を1回押す**。設定変更は不要、開いて保存するだけ。
（「design casa 初期データ」→「固定ページの文章を作り直す」でも同じ効果）
v1.4.1 に入れ替えれば以後は自動修復されるため、この操作は不要になります。
...
**原因**：タクソノミーのリライト規則が `rewrite_rules` オプションから失われていた。
絞り込みURLは CPT スラッグの下に入れ子（`works/feature`・`spec/category`）のため、
**CPT側は生きたまま絞り込みだけが404**になり、気づきにくい。
...
判定ロジックは「規則あり／なし／空／未生成」の4パターンでスタブ検証済み。
※ WordPress 実機での動作は未検証（ローカルにWP環境がないため）。本番反映後に
`/works/feature/hiraya/` が200になることを確認すること。
