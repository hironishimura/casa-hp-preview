...
施工例の写真を**1投稿あたり3〜5枚**のカルーセルにまとめ、**毎週火曜と金曜の10時**に
自動で Instagram へ投稿する仕組みです。1棟を2回に分けて出すので、
67件で**128投稿**のストックがあります。

投稿ペースは日付で自動的に切り替わります。

| 期間 | ペース |
|---|---|
| 〜2026-11-04 | **1日おき**（偶数日）— 開設直後にストックを厚くするため |
| 2026-11-05〜 | **週2回**（火・金） |

移行期間は**曜日を問いません**（土日も投稿します）。
月末で連日にならないよう、cron ではなくジョブ側の日付判定で制御しています。

この配分で、127投稿を使い切るのは **2027年10月ごろ**です。
...
`count` に数字を入れると、その件数を**3分間隔で連続投稿**します（`post` のときのみ）。
アカウント開設直後にストックを作るための機能です。短時間に大量投稿すると
スパムとみなされるおそれがあるため、一度に10件程度までにしてください。
...
**ペースの切替日**は「今日が投稿日か判定する」ステップの `SWITCH='2026-11-05'` です。
この日付を変えれば、1日おきの期間を延ばしたり縮めたりできます。
...
`scripts/instagram/settings.json` の `"1投稿あたりの最大枚数"` を変えます。
端数だけの貧相な投稿ができないよう、**均等に割ります**
（既定の5なら、7枚の施工例は 5+2 ではなく 4+3 の2回に分かれます）。
10 にすると1棟1投稿に戻ります。
